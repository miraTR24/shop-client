...
const ProductForm = () => {
    ...
    const setPrice = (price: string) => {
        ...
        setProduct({ ...product, price: Number.isNaN(convertedPrice) ? 0 : Number(convertedPrice.toFixed(2)) });
    };

    const setShop = (shop: any) => {
        setProduct({ ...product, shop: shop.name === 'Aucune' ? null : shop });
    };
    ...
    return (
        ...
        >
            <Typography variant={isSmallScreen ? 'h4' : 'h2'} sx={{ marginBottom: 3, textAlign: 'center' }}>
                {isAddMode ? 'Ajouter un produit' : 'Modifier le produit'}
            </Typography>

            <FormControl sx={{ width: '100%', mb: 3 }}>
                <Divider>Nom du produit</Divider>
                <Box sx={{ display: 'flex', flexDirection: isSmallScreen ? 'column' : 'row', gap: 2, mt: 2, mb: 4 }}>
                    <TextField
                        autoFocus
                        required
                ...
                <Box sx={{ display: 'flex', flexDirection: isSmallScreen ? 'column' : 'row', gap: 2, mt: 2 }}>
                    <TextField
                        required
                        type="number"
                        label="Prix"
